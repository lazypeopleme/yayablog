...
const actions: ActionTree<RootStateTypes, any> = {
  ...
  GET_ARTICLES_LIST({ commit }, params) {
    // 获取文章列表
    axios
      .get("http://localhost/www/yayablog/getArticlesList.php", {
        params: { page: 1, size: 6, ...params }
      })
      .then(res => {
        const { content = [], ...pageInfo } = res.data;
        commit("GET_ARTICLES_LIST", { content, pageInfo });
      });
  },
  ...
  SELECT_IMG({}, params: any) {
    // 查询图片列表
    return axios.get("http://localhost/www/yayablog/getImgList.php", {
      params
    });
  },
  UPDATE_ARTICLE({}, params: any) {
    // 修改文章内容
    return axios.post(
